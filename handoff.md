...
- **보조 프로그램 카탈로그**: 확장·단축키·툴 등록 (`tools/aux_tools.py`). 카드 UI(아이콘·태그·도메인·행 높이 균일).
- **AI 꿀팁 컬렉션**(2026-06-27 신설): 짧고 즉시 따라할 사용 노하우 모음 (`tools/tips_tools.py`, `data/ai_tips.json`). `"[내용] 꿀팁 추가해줘"` → `add_tip`. 보조 프로그램(도구 링크)과 분리, URL 불필요. 앱 **💡 AI 꿀팁** 탭.
- **PPT 슬라이드**: Canva MCP 또는 `tools/pptx_maker.py`(python-pptx, 흑백, Pretendard).

### UI (2026-06-27 개편)

- **보라색 SaaS 테마**: 액센트 `#7C5CFC`, 연보라 캔버스, 16px 둥근 카드, 그라데이션 히어로 배너(`_hero()`).
- **좌측 세로 네비로 일원화**: 상단 세그먼트 제거 → 사이드바가 메인 메뉴(`_render_sidebar_nav`). 활성 항목 연보라 강조. 다크모드 토글·역할·로그아웃은 하단. (슬라이드 덱은 PPTX 흑백 일치라 무수정)
- **물결표(~) 처리**: 본문 마크다운은 `_escape_tilde()`로 GFM 취소선 깨짐 방지 — 숫자 범위는 엔대시(–), 그 외 `~`는 리터럴. 코드·링크 URL은 보호.
